...
function ActiveProjectDetails({
  ...
  return (
    <div className="mt-10 max-w-md">
      ...
      <p className="gallery-prose mt-5">{project.shortDescription}</p>
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-14 lg:flex-row lg:items-start lg:justify-between lg:gap-24">
            ...
            {projects.map((project, index) => (
              ...
              >
                <TextStaggerHover
                  index={index}
                  text={project.title}
                  className="cursor-pointer font-serif text-3xl font-normal tracking-[0.03em] text-foreground sm:text-4xl lg:text-5xl lg:leading-none"
                />
              </Link>
            ))}
          </div>
        ...
        <HoverSliderImageWrap className="relative aspect-[4/5] w-full max-w-sm sm:max-w-md lg:max-w-xl lg:shrink-0">
          {projects.map((project, index) => (
            <HoverSliderImage
              key={project.slug}
              index={index}
